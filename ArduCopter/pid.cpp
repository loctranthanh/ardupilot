...
PID::PID()
{
    ...
    pid_param.ipart = 0;
    pid_param.dpart = 0;
    pid_param.dpart_alpha = 0.2;
    pid_param.u = 0;
    pid_param.u_ = 0;
    pid_param.flag_first_time = true;
    for (i=0;i<DPART_N_AVG;i++)
    {
        pid_param.dpart_tmp[i] = 0;
    }
}
...
float PID::pid_process(float error,uint32_t time)
{
    ...
    else
    {
        ...
        pid_param.time = time;
        pid_param.Ts = ((float)(pid_param.time - pre_time))/1000.0f;
        ...
        pid_param.dpart = 0;
        for (i=DPART_N_AVG-1;i>0;i--)
        {
            pid_param.dpart_tmp[i] = pid_param.dpart_tmp[i-1];
            pid_param.dpart += pid_param.dpart_tmp[i]; 
        }
        ...
        pid_param.u = pid_param.u_ + pid_param.ppart
                + pid_param.Ki * pid_param.Ts * pid_param.e
                + pid_param.dpart;
#else        
        pid_param.dpart_tmp[0] = (pid_param.Kd / pid_param.Ts) * (pid_param.e - pid_param.e_);
        pid_param.dpart += pid_param.dpart_tmp[0];
        pid_param.dpart /= DPART_N_AVG;     
        pid_param.dpart = pre_dpart + pid_param.dpart_alpha * (pid_param.dpart - pre_dpart);   
